<?php
    //page affichant les participations aux évènements par personnes...point6
    require_once("auth/EtreAuthentifie.php");
    // if ($idm->getRole()!="admin") {
    //     redirect($pathFor["acceuil.php"]);
    // }
    require_once("auth/loader.php");
    $title = "Participation de la personne";
    if (!isset($_GET["pid"])) {
    ...
    $SQL= "SELECT  DISTINCT personnes.nom,personnes.prenom FROM participations INNER JOIN personnes on participations.pid=personnes.pid WHERE participations.pid=? ";
    $st = $db->prepare($SQL);
    $st->execute([$pid]);
    if ($st->rowCount()==0) {     
    ...
    $row = $st->fetch();
    $h = "Participations de: ".$row['nom']." ".$row['prenom'];
    include("navadmin.php");

    

    $SQL= "SELECT  * FROM evenements INNER JOIN categories ON evenements.cid = categories.cid INNER JOIN participations ON participations.eid=evenements.eid WHERE participations.pid=? ORDER BY participations.date DESC";
    $st = $db->prepare($SQL); 
    if(!$st->execute([$pid])){
        header('Location: tbpers.php');
